...
class UniFileExtensionsTest {
    ...
    fun `uses rename when supported`() {
        ...
        source.renameToOrCopy("001.jpg") shouldBeSameInstanceAs source
    ...
    fun `copies file and deletes source when rename is unsupported`() {
        ...
        source.renameToOrCopy("001.jpg") shouldBeSameInstanceAs target
        output.toByteArray().toList() shouldBe contents.toList()
    ...
    fun `copies directories recursively`() {
        ...
        source.renameToOrCopy("chapter") shouldBeSameInstanceAs target
        output.toByteArray().toList() shouldBe contents.toList()
    }
    ...
    fun `fails without overwriting an existing target`() {
        ...
        shouldThrow<IOException> { source.renameToOrCopy("source.jpg") }
